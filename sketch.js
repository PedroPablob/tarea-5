...
class Firework {
  ...
  update() {
    ...
    for (let i = this.particles.length - 1; i >= 0; i--) {
      this.particles[i].applyForce(createVector(0, 0.1)); // Hacer que la caída sea más lenta
  ...
  explode() {
    for (let i = 0; i < 100; i++) {
      ...
      let particle = new Particle(this.firework.pos.x, this.firework.pos.y, false, color(r, g, b), particleSize);
...
class Particle {
  constructor(x, y, firework, pcolor, size) {
    ...
    if (this.firework) {
      ...
      this.vel = p5.Vector.random2D().mult(random(1, 3)); // Velocidad aleatoria para las partículas
      this.lifespan = 255;
    }
